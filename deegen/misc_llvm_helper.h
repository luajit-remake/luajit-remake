...
public:
    ...
    llvm::Constant* WARN_UNUSED Get(size_t index)
    {
        ...
        else
        {
            ...
            llvm::ArrayType* art = dyn_cast<llvm::ArrayType>(m_value->getType());
...
// Print "{ ...values... }"
//
inline void PrintUInt8ArrayContentAsCppCode(FILE* fp, const std::vector<uint8_t>& contents, size_t indent = 0)
{
    if (contents.size() == 0)
    {
        fprintf(fp, "{}");
    ...
    fprintf(fp, "{\n");
    fprintf(fp, "%s    ", indentStr.c_str());
    for (size_t i = 0; i < contents.size(); i++)
    {
        fprintf(fp, "%d", static_cast<int>(contents[i]));
...
// Print CPP code to define a constant uint8_t array
//
inline void PrintCppCodeToDefineUInt8Array(FILE* hdrFp, FILE* cppFp, const std::vector<uint8_t>& contents, std::string name)
{
    fprintf(hdrFp, "extern const uint8_t %s[%d];\n", name.c_str(), static_cast<int>(contents.size()));
    fprintf(cppFp, "extern const uint8_t %s[%d];\n", name.c_str(), static_cast<int>(contents.size()));
    fprintf(cppFp, "const uint8_t %s[%d] = ", name.c_str(), static_cast<int>(contents.size()));
    PrintUInt8ArrayContentAsCppCode(cppFp, contents);
    fprintf(cppFp, ";\n\n");
}
