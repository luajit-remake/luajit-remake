...
always_put_same_value_in_two_tables = false
do_not_insert_non_vector_index = false
num_repeats = 1

function execute_one_putbyval_test_op(o, gold, o2, gold2)
	local dice = rand() % 30
	local key
	...
	for iter = 1, num_repeats do
		...
		if (rand() % 3 ~= 0 and (not always_put_same_value_in_two_tables)) then
			newValue = get_value_to_put()
		end
		if (not test_put_by_val_impl(o2, gold2, key, newValue)) then
			return false
		end
	end
	return true
...
function exeucte_one_test_op(o, gold, o2, gold2)
	local dice = rand() % 13
	if (dice < 10) then
		return execute_one_putbyval_test_op(o, gold, o2, gold2)
	end
	...
	if (dice == 10) then
		...
		if (idx <= 50) then
			for iter = 1, num_repeats do
				...
				if (not test_put_by_imm(o2, gold2, idx, newValue)) then
					return false
				end
			end
			return true
	...
	for iter = 1, num_repeats do
		...
		if (not test_put_by_imm(o2, gold2, rand() % 51, newValue)) then
			return false
		end
	end
	return true
...
for numTests = 1, 200 do
	...
	if (rand() % 2 == 0) then
		...
		always_put_same_value_in_two_tables = true
	end
	
	local success = true
	for testOp = 1, 500 do
		if (not exeucte_one_test_op(o, gold, o2, gold2)) then
		...
		if (testOp % 10 == 0) then
			if (not check_table_identical(o, gold)) then
				print('check_table_identical (o) failed!')
				success = false
				break
			end
			if (not check_table_identical(o2, gold2)) then
				print('check_table_identical (o2) failed!')
				success = false
				break
			end
		end
