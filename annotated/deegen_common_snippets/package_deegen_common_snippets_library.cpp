...
std::pair<std::string /*name*/, std::pair<std::string /*IR*/, int /*kind*/>> WARN_UNUSED GetProcessedFileContent(const std::string& fileName, const std::string& fileData)
{
    ...
    GlobalVariable* gvKind = module->getGlobalVariable(snippetKindVarName);
    ReleaseAssert(gvKind != nullptr);
    ...
    if (snippetKind != 1 /*importable declaration*/)
    {
    ...
    return std::make_pair(snippetName, std::make_pair(fileContents, snippetKind));
}
...
void DoWork(bool selfBinaryChanged, const std::vector<std::string>& files, const std::string& outputDirectory)
{
    ...
    for (const std::string& file : files)
    {
        ...
        declCode += std::string("extern const int g_generated_deegen_common_snippet_kind_") + id + ";\n";

        mapInitCode += std::string("{ g_generated_deegen_common_snippet_name_") + id + ", std::make_pair(&g_generated_deegen_common_snippet_" + id + ", g_generated_deegen_common_snippet_kind_" + id + ") },\n";
        ...
        std::pair<std::string /*name*/, std::pair<std::string /*IR*/, int /*kind*/>> resultData = GetProcessedFileContent(file /*fileName*/, originData);
        ...
        size_t len = resultData.second.first.length();
        ...
        {
            fprintf(o.fp(), "\\x%02x", static_cast<unsigned int>(static_cast<uint8_t>(resultData.second.first[i])));
            if (i % 32 == 31 && i < len - 1)
            {
                fprintf(o.fp(), "\"\n\"");
            }
        ...
        fprintf(o.fp(), "extern const int g_generated_deegen_common_snippet_kind_%s;\n", id.c_str());
        ...
        fprintf(o.fp(), "const int g_generated_deegen_common_snippet_kind_%s = %d;\n", id.c_str(), resultData.second.second);
...

    {
        ...
        fprintf(o.fp(), "std::unordered_map<std::string, std::pair< const std::pair<const char*, size_t>*, int > > g_deegenCommonSnippetLLVMIRMap {\n");
