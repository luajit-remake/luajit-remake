...
local function newcell()
	return { child1 = nil, child2 = nil, child3 = nil, child4 = nil, color = gray }
end
...
local function clip(q,xmin,xmax,ymin,ymax,o,oxmin,oxmax,oymin,oymax)
	...
	if iymin<iymax then
		if q.child1==nil then
			...
			clip(q.child1,xmin,xmid,ymid,ymax,o,oxmin,oxmax,oymin,oymax)
			clip(q.child2,xmid,xmax,ymid,ymax,o,oxmin,oxmax,oymin,oymax)
			clip(q.child3,xmin,xmid,ymin,ymid,o,oxmin,oxmax,oymin,oymax)
			clip(q.child4,xmid,xmax,ymin,ymid,o,oxmin,oxmax,oymin,oymax)
		end
	end
end
...
local function update(q,xmin,xmax,ymin,ymax)
	if q.color==gray then
		if q.child1==nil then
			...
			local xmid=(xmin+xmax)/2.0
			local ymid=(ymin+ymax)/2.0
			update(q.child1,xmin,xmid,ymid,ymax)
			update(q.child2,xmid,xmax,ymid,ymax)
			update(q.child3,xmin,xmid,ymin,ymid)
			update(q.child4,xmid,xmax,ymin,ymid)
		end
	end
end
...
local function area(q)
	if q.child1==nil then
		...
		local g1,b1=area(q.child1)
		local g2,b2=area(q.child2)
		local g3,b3=area(q.child3)
		local g4,b4=area(q.child4)
		return (g1+g2+g3+g4)/4.0, (b1+b2+b3+b4)/4.0
	end
end

local function colorup(q)
	if q.child1~=nil and q.color==gray then
		local c1=colorup(q.child1)
		local c2=colorup(q.child2)
		local c3=colorup(q.child3)
		local c4=colorup(q.child4)
		if c1==c2 and c1==c3 and c1==c4 then
if c1~=gray then
			q.child1=nil; --q.child2=nil; q.child3=nil; q.child4=nil
N=N+1 end
			q.color=c1
		end
	end
...
local function save(q,xmin,ymin,N)
	if q.child1==nil or N==1 then
		output(xmin,ymin,N,q.color)
	else
		N=N/2
		local xmid=xmin+N
		local ymid=ymin+N
		save(q.child1,xmin,ymin,N)
		save(q.child2,xmid,ymin,N)
		save(q.child3,xmin,ymid,N)
		save(q.child4,xmid,ymid,N)
	end
end
